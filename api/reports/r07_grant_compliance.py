...
SQL = """
    SELECT
        ...
        gg.grant_year,
        gg.grant_amount,
        gg.amount_approved_ytd,
        ROUND(
            gg.amount_approved_ytd * 100.0 / NULLIF(gg.grant_amount, 0), 1
        )                                                                  AS utilization_pct,
        gg.status,
        p.program_name,
        ...
        ROUND(
            gg.grant_amount / NULLIF(COUNT(DISTINCT pe.beneficiary_id), 0), 2
        )                                                                  AS grant_cost_per_client
...
def r07_grant_compliance():
    data        = query(SQL)
    summary_row = query(SQL_SUMMARY)[0]

    total_clients   = sum(r["beneficiaries_served"] or 0 for r in data)
    total_outcomes  = sum(r["outcomes_achieved"]    or 0 for r in data)

    return ReportResponse(
        report_id="r07",
        report_name="Grant Compliance Overview",
        description="Grant utilization rates, clients served per grant, and outcomes achieved. Cross-references grant funding (Business Central) with program delivery (Newtract).",
        sources=["Business Central", "Newtract"],
        generated_at=datetime.now().isoformat(),
        summary={
            **summary_row,
            "total_clients_funded":   total_clients,
            "total_outcomes_funded":  total_outcomes,
        },
        data=data,
        chart=ChartSpec(
            type="grouped_bar",
